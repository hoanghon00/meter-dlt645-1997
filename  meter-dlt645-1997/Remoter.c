...
void Remoter_Cmd_Decode(INT8U Cmd)
{
  ...
  {     
    case IR_LIGHT_ON:
      ...
      break;
    case IR_UP_KEY:
      Key_Value_Pub.Bit.UpKey=1;
      break;
    case IR_DOWN_KEY:
      Key_Value_Pub.Bit.DownKey=1;
      break;
    case IR_LEFT_KEY:
      Key_Value_Pub.Bit.LeftKey=1;
      break;
    case IR_RIGHT_KEY:
      Key_Value_Pub.Bit.RightKey=1;
      break;
    case IR_ENTER_A:
    case IR_ENTER_B:
      Key_Value_Pub.Bit.AB_Key=1;
      break;
  default:
    if(Cmd<=IR_MAX_NUM)
    {
      Set_Mode_DisCode(Cmd);      
    }
  }
}
void Remote_Key_Proc(void)
{  
  if(CHK_VAR_CS_PUCK(Irda_Decode_Ctrl)==0)    //有错误发生
  {
    CLEAR_VAR_PUCK(Irda_Decode_Ctrl);
    ASSERT(A_WARNING,0);
    return ;
  }
